<?php
    ...
    $systemSettingReadAccess = $globalModel->checkAccessRights($userID, $pageID, 'read');
    $systemSettingCreateAccess = $globalModel->checkAccessRights($userID, $pageID, 'create');
    $systemSettingWriteAccess = $globalModel->checkAccessRights($userID, $pageID, 'write');
...
<html lang="en" dir="ltr" data-bs-theme="light" data-color-theme="Blue_Theme" data-layout="vertical">
    ...
    <body>
            ...
            <div class="page-wrapper">
                ...
                <div class="body-wrapper">
                    <div class="container-fluid">
                                                ...
                                                <ol class="breadcrumb fs-2">
                                                    <?php
                                                        ...
                                                        if(!$newRecord && !empty($detailID)){
                                                            echo '<li class="breadcrumb-item" id="system-setting-id">'. $detailID .'</li>';
                                                        }
                        ...
                        <?php
                            if($newRecord){
                                require_once('components/system-setting/view/_system_setting_new.php');
                            }
                            else if(!empty($detailID)){
                                require_once('components/system-setting/view/_system_setting_details.php');
                            }
                            else{
                                require_once('components/system-setting/view/_system_setting.php');
                            }
                        ?>
                    </div>
                </div>
        ...
        <?php 
            if($newRecord){
                $scriptLink = 'system-setting-new.js';
            }
            else if(!empty($detailID)){
                $scriptLink = 'system-setting-details.js';
            }
            else{
                $scriptLink = 'system-setting.js';
            }

            echo '<script src="./components/system-setting/js/'. $scriptLink .'?v=' . rand() .'"></script>';
        ?>
    </body>
</html>
